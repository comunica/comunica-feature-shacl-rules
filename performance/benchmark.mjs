...
function runEyesharl(src) {
  if (!eyesharl) {
    return { ms: Number.NaN, count: 'N/A', iterations: '-' };
  }
  const t0 = performance.now();
...
for (const file of filteredFiles) {
  ...
  try {
    ...
    comunica = { ms: Number.NaN, count: 'ERR', constructCalls: '-', timing: null, error: e.message };
  }
  try {
    eye = runEyesharl(src);
  } catch (e) {
    eye = { ms: Number.NaN, count: 'ERR', iterations: '-', error: e.message };
  }
